...
# Route publique pour la liste des employés (pour le pointage)
@employee_bp.route('/employees/list', methods=['GET'])
def get_employees_list():
    """Récupérer la liste simplifiée des employés actifs pour le pointage (accès public)"""
    try:
        employees = Employee.query.filter_by(is_active=True).order_by(Employee.last_name, Employee.first_name).all()
        
        # Retourner seulement les informations nécessaires pour le pointage
        employees_list = []
        for emp in employees:
            employees_list.append({
                'employee_number': emp.employee_number,
                'display_name': f"{emp.first_name} {emp.last_name}",
                'full_display': f"{emp.employee_number} - {emp.first_name} {emp.last_name}"
            })
...
@admin_required
def create_employee():
    """Créer un nouvel employé (admin seulement) - AMÉLIORATION: numéro libre"""
    try:
        data = request.get_json()
        ...
        # AMÉLIORATION: Permettre n'importe quel format de numéro d'employé
        employee_number = data['employee_number'].strip()
        if not employee_number:
            return jsonify({'error': 'Le numéro d\'employé ne peut pas être vide'}), 400
        
        # Vérifier l'unicité du numéro d'employé (insensible à la casse)
        if Employee.query.filter(Employee.employee_number.ilike(employee_number)).first():
            return jsonify({'error': 'Ce numéro d\'employé existe déjà (insensible à la casse)'}), 400
        ...
        if len(data['password']) < 3:  # Réduire la contrainte pour plus de flexibilité
            return jsonify({'error': 'Le mot de passe doit contenir au moins 3 caractères'}), 400
        
        # Créer l'employé
        employee = Employee(
            employee_number=employee_number,  # Utiliser le numéro tel que saisi
...
def update_employee(employee_id):
    """Mettre à jour un employé (admin seulement) - AMÉLIORATION: numéro libre"""
    try:
        data = request.get_json()
        employee = Employee.query.get_or_404(employee_id)
        ...
        if 'employee_number' in data:
            # AMÉLIORATION: Permettre n'importe quel format
            new_number = data['employee_number'].strip()
            if not new_number:
                return jsonify({'error': 'Le numéro d\'employé ne peut pas être vide'}), 400
            
            # Vérifier l'unicité (insensible à la casse)
            existing = Employee.query.filter(
                Employee.employee_number.ilike(new_number),
        ...
        if 'password' in data and data['password']:
            if len(data['password']) < 3:  # Contrainte réduite
                return jsonify({'error': 'Le mot de passe doit contenir au moins 3 caractères'}), 400
